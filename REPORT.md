...
#### E. Minimal HITL, not full live handoff
The current implementation supports a minimal human approval step via `humanApproval`, which is a terminal yes/no approval gate.

**Trade-off:**
- Advantage: simple safety stop for risky or uncertain actions.
- Cost: this is not yet a full same-session human handoff with resume and action recording.
...
#### A. Typed step kinds
Replay only executes a known set of step kinds (`goto`, `click`, `fill`, `press`, `waitFor`, `extractText`, `assertText`, `screenshot`, etc.). This limits ambiguity.
...
Discovery includes a repeated-action check to avoid getting stuck in simple loops. If the same action is proposed again with no apparent change, the system can block it and use a heuristic override or fail fast.

#### E. Human approval for risky or uncertain steps
Discovery supports a minimal `humanApproval` step that pauses for terminal approval. This is currently an approval gate, not a full same-session intervention handoff.
